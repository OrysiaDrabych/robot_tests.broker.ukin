...
Підготувати клієнт для користувача
    ...
    Run Keyword If    '${ARGUMENTS[0]}' != 'ukin_Viewer'    Login    ${ARGUMENTS[0]}
...
Додати предмет закупівлі
    ...
    ukin.Пошук тендера по ідентифікатору  ${username}  ${tender_uaid}
...
Завантажити документ
    ...
    ...    ${ARGUMENTS[2]} == ${TENDER}
    ukin.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}    ${ARGUMENTS[2]}
    Click Element    id=lot-update-btn
...
Оновити сторінку з тендером
    ...
    Switch Browser    ${ARGUMENTS[0]}
    ukin.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}    ${ARGUMENTS[1]}
...
Отримати інформацію із предмету
    ...
    ${return_value}=    Run Keyword And Return    ukin.Отримати інформацію із тендера    ${username}    ${tender_uaid}    ${field_name}
    [Return]    ${return_value}
...
Отримати інформацію про dgfDecisionDate
    ...
    ${return_value}=    ukin_service.convert_date    ${date_value}
    [Return]    ${return_value}
...
Внести зміни в тендер
    ...
    Choose File    id = file-type-input    /home/wolodymyr/robot_tests/src/robot_tests.broker.ukin/test.txt
    Click Element    id=submit-auction-btn
    Sleep    10
...
Задати питання
    ...
    ukin.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}    ${ARGUMENTS[1]} == tenderUaId
    Wait Until Page Contains Element    id = auction-view-btn
    Click Element    id = auction-view-btn
...
Задати запитання на тендер
    ...
    ukin.Пошук тендера по ідентифікатору    ${username}    ${tender_uaid}
    ukin.Задати питання    ${username}    ${tender_uaid}    ${question}
...
Відповісти на питання
    ...
    ukin.Перейти до сторінки запитань    ${username}  ${tender_uaid}
...
Отримати документ
    ...
    ukin.Пошук тендера по ідентифікатору    ${username}    ${doc_id}
    sleep    1
...
Підтвердити підписання контракту
    ...
    Sleep    1
    ukin.Завантажити угоду до тендера    ${username}    ${tender_uaid}    1    ${filepath}
