...
Підготувати клієнт для користувача
    ...
    Run Keyword If    '${username}' != 'ukin_Viewer'    Login    ${username}
...
Перейти на сторінку тендера
    ...
    Run Keyword Unless    ${present}    ukin.Пошук тендера по ідентифікатору    ${username}    ${tender_uaid}

Отримати інформацію із тендера
    [Arguments]  ${username}  ${tender_uaid}  ${fieldname}
    Run keyword    ukin.Перейти на сторінку тендера  ${username}  ${tender_uaid}
    ${return_value}=  Run Keyword  ukin.Отримати інформацію про ${fieldname}
    [return]  ${return_value}
